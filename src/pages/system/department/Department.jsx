...
const DepartmentPage = () => {
  ...
  const onEditFormFinish = async (values) => {
    try {
      const res = await UpdateDepartmentRequest(values);
      if (res.code === 200) {
        message.success('部门修改成功', 1).then(() => {
          navigate(0);
        });
      } else {
        message.error(res.message);
  ...
  useEffect(() => {
    let ids = [];
    if (departmentInfo?.manage_users?.length) {
      ids = Object.keys(departmentInfo?.manage_users).map(key => departmentInfo?.manage_users[key].id);
    }
  ...
  const onAddFormFinish = async (values) => {
    try {
      const res = await AddDepartmentRequest(values);
      if (res.code === 200) {
        message.success('部门添加成功', 1).then(() => {
  ...
  return (
    <>
                  ...
                  {departmentInfo?.name ? (
                    <Form {...layout} form={editForm} name="edit-menu-info" onFinish={onEditFormFinish}>
                      ...
                      <Form.Item name="parent_id" label="父级部门" extra={formExtra.parent}
                                 rules={[{ required: true }]}>
                        <Select showSearch placeholder="选择父级部门" optionFilterProp="children"
                                filterOption={parentDepartmentFilterOption} options={parentDepartmentSelectItems} />
                      </Form.Item>
                      <Form.Item name="manage_users" label="部门负责人" extra={formExtra.manager}>
                        <Select showSearch mode="multiple" placeholder="选择部门负责人" optionFilterProp="children"
                                filterOption={manageUserFilterOption} options={manageUserSelectItems} />
                      </Form.Item>
                      <Form.Item {...tailLayout}>
                        <Space>
                          ...
                          {CurrentUserInfo?.role_id === 1 ? (
                            <Popconfirm title="是否确定删除该部门？" okText="确定" okType="danger" cancelText="取消"
      ...
      {/*添加部门表单*/}
      <Modal title="添加部门" className="admin-modal-form" centered open={showDepartmentAddModal} maskClosable={false}
             footer={null} onCancel={() => setShowDepartmentAddModal(false)}>
