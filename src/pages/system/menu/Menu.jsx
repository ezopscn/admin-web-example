  ...
  return (
    <ul>
      <li>
        ...
        <ul>
          <li>为了保证系统数据安全性，只有系统预留的超级管理员角色才具备菜单删除权限，且被删除的菜单不允许还有子菜单。</li>
          <li>
            菜单的删除属于软删除，数据库中数据依然会存在。这也导致如果后续新添加一个同名的或同路径的菜单时，系统会提示添加失败。遇到该问题，可以联系系统管理员对数据库中已删除的同名或同路径的菜单数据的字段进行修改调整，避免冲突。
          </li>
        </ul>
      </li>
      <li>
